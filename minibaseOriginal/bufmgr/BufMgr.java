...
public class BufMgr implements GlobalConst
{
			...
			PagePinnedException, BufMgrException, IOException
	{
		...
		frameNo = hashTable.lookup(pin_pgid);//Busca la p�gina en el buffer pool	( tabla de hash )

		if (frameNo < 0)
		{ // Not in the buffer pool

			frameNo = replacer.pick_victim(); // Busca un frame para alojar
			if (frameNo < 0)
			{
				page = null;
			...
			bst = hashTable.remove(frmeTable[frameNo].pageNo);//Elimina la asociacion pagina-frame en la tabla de hash
			if (bst != true)
			{
			...
			bst2 = hashTable.insert(pin_pgid, frameNo);//Inserta la nueva asociaci�n en la tabla de hash

			(frmeTable[frameNo].pageNo).pid = pin_pgid.pid;
			frmeTable[frameNo].dirty = false;				//Inserta la asociaci�n en la frmeTable
			...
			if (needwrite == 1)
			{
				write_page(oldpageNo, apage);//Si necesita escribir la p�gina vieja que fue removida, lo hace
			} // end of needwrite..

			//Si la p�gina no est� vac�a la lee de disco y la mete en el bufPool
			if (emptyPage == false)
			{
				try
				{
		...
		{ // la p�gina est� en el buffer pool ( frameNo > 0 )
